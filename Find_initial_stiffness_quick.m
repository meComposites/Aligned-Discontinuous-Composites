function Pre_defect_stiffness=Find_initial_stiffness_quick(n_rows_outer_layer,n_columns_outer_layer,RVE_outer_layer,E_fibres,MFT1,MFT2,Mt,MSS,GiicmAvg,G,RandomLength,EpsilonGrid,EpsilonMax,Mint,Mfib,Mfail,MNumNeighbour,lf,ds,Vf,tm,inter_fibre_variability)
...
%         EInt(k)=Calculate_overlap_stiffness(EE_int,signDeltaK*DE_int,ET_int,signDeltaK*DT_int,tm_int,lk,G);
%         lInt(k)=lk;
        [Epsilon,Sigma,EInt]=Toughness_or_strength_failure(EE_int,signDeltaK*DE_int,ET_int,signDeltaK*DT_int,tm_int,lk,Sm_int,GiicmAvg,G,ds);
        SSint{k}={Epsilon*100,Sigma,EInt,[],lk};
    end
    
%     [xSeries,ySeries]=CombineInSeriesStiffness(EInt,lInt,ds,lf,EpsilonGrid(2)); % OKKKKKKK!
    [xSeries,ySeries]=CombineInSeries(SSint,ds,lf,EpsilonMax);
...
%             dEpsilon=max(10^(-6),du/lf*100); % in (%)
            ...
            Epsilon(end+1,1)=Epsilon(end,1)+Epsilon(2,1); % in (%)
    ...
    %% Combination in series
    function [xSeries,ySeries]=CombineInSeriesStiffness(EInt,lInt,ds,lf,deltaEpsilon) %{xInt,yInt,EInt,deInt,lk} in each
...
%         for ki=1:n
%             Emaxn=[Emaxn max(EInt(ki))];
%         end
        % FJ - finds minimum stiffness of all overlaps
        [Smax,iMin]=min(EInt*deltaEpsilon/100);
        % changed with SP updates to effects of randomness
        %         nSmax=Smax/ds;
        % number of increments equals minimum strength / ds
%         nSmax=floor(Smax/ds);
...
%         else
            ...
            for kl=1:n
                % displacement equals previous displacements + strain times
                % length of overlap
%                 u=u+EInt{kl}{1}(1:nSmax+1)/100*EInt{kl}{5}; % in mm
                u=u+deltaEpsilon/100*lInt(kl);
            end
            % find epsilon from displacement of total interaction
            Epsilon=[0 u/lf*100]'; % in (%)
...
%failure
function [EInt]=Calculate_overlap_stiffness(SumE,DeltaE,SumT,DeltaT,tI,Lo,GI)
DeltaT=-DeltaT;
DeltaE=-DeltaE;
...
try
    ...
    EInt=E_RoM*beta/(1+(delta/(lambda*Lo*0.5*tanh(lambda*Ll))));
...
function [Epsilon,Sigma,E]=Toughness_or_strength_failure(SumE,DeltaE,SumT,DeltaT,tI,Lo,SI,GiicI,GI,deltaSigma)
DeltaT=-DeltaT;
DeltaE=-DeltaE;
