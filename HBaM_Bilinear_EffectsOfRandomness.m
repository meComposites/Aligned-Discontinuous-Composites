function [xInt,yInt,EInt,deInt] = HBaM_Bilinear_EffectsOfRandomness(EE,DE,ET,DT,tm,L,SmIn,Giicm,G)
...
[G,Sm,gm,Y] = inputs_2Stiffness_VariableGiicSm(EE,DE,ET,DT,tm,Giicm,SmIn,G);
...
[gpzMax] = gMatrix_Bilinear_EffectsOfRandomness(N,type,Y,gm,Sm,ET,DT,tm,EE,DE);
LnR=-1;
    ...
    if nL == 0 || nR == 0  % Only one subdomain on one side
        ...
        if LeftActivation
            ...
            if (size(gpzMax,2)~=max(State_L(end),State_R(end)))
                [g01,~,~]= fminsearch(@find_g0_act_L,ginf,opts);
        ...
        elseif RightActivation
            ...
            gsup=gpzMax(i,State_R(end),State_L(end));
            if (size(gpzMax,2)~=max(State_L(end),State_R(end)))
                [g01,~,~]= fminsearch(@find_g0_act_R,ginf,opts); %%%%%%%%%%%% TEST
        ...
        % FJ - changed to speed up code
%         if (State_L(end)+State_R(end)>=2*N+1) % No possible activations: deactivation of center subdomain
...
%             LeftActivation = 1-RightActivation;
            ...
            if LeftActivation
                ...
                if length_act < L/2 && jkl < 0.01 % Left activation possible
                    State_L=[State_L (State_L(end)+1)];
            ...
            elseif RightActivation
                ...
                if length_act < L/2 && jkl < 0.01 % Right activation possible
                    State_R=[State_R (State_R(end)+1)];
                    
                    g0=[];
                    gsup=gpzMax(i,State_R(end),State_L(end));
                    if (size(gpzMax,2)~=max(State_L(end),State_R(end)))
                        [g01,~,~]= fminsearch(@find_g0_act_R,ginf,opts);
                        [g0,jkl,~]=fminbnd(@find_g0_act_R,ginf,gsup,opts);
...
%     AAE(i)=EEE; % Collection of stiffness
    Epsilon=SS_steps(:,1);
    Sigma=SS_steps(:,2);
...
%         for gj=exp(log(g_crit(end-1)):(log(g_crit(end))-log(g_crit(end-1)))/ng:log(g_crit(end))) % evolution of g between the two last critical values
        % FJ - changed to speed up code (stops duplicating calculations at end points of each sub-domain)
        for gj=exp(log(g_crit(end-1))+(log(g_crit(end))-log(g_crit(end-1)))/ng:(log(g_crit(end))-log(g_crit(end-1)))/ng:log(g_crit(end))) % evolution of g between the two last critical values
            xFieldR=[];
            ShearStressR=[];
            PsiStressR=[];
            ...
            %S0iR=interp1q([0; gm],[0; Sm],g0iR);
            S0iR=(g0iR<=gm(1))*g0iR/gm(1)*Sm(1)+(gm(1)<g0iR)*(g0iR<gm(2))*Sm(1)*(1-(g0iR-gm(1))/(gm(2)-gm(1)));
            P0iR=0;
            sumPreviousR=0;
            ...
            %S0iL=interp1q([0; gm],[0; Sm],g0iL);
            S0iL=(g0iL<=gm(1))*g0iL/gm(1)*Sm(1)+(gm(1)<g0iL)*(g0iL<gm(2))*Sm(1)*(1-(g0iL-gm(1))/(gm(2)-gm(1)));
            P0iL=0;
            sumPreviousL=0;
...
%             end
            ...
            %[LnR2,jkl2,~]= fmincon(@find_P_fixed_g0,(min(LRA,LRB)+max(LRA,LRB))/2,[],[],[],[],min(LRA,LRB),max(LRA,LRB),[],opts);
            %1;
...
%             Xb3Dplot=[Xb3Dplot;Xb3DplotTemp'];
    ...
    function find_P_fixed_g0 = find_P_fixed_g0(LnR_guess_fixed_g0)
        ...
        [PL_end_plot]=field_2Stiffness_nonIterative_P_EffectsOfRandomness(State_L_previous,State_L_previous(end),EE,DE,G,Sm,gm,Y,0,ET,DT,tm,...
            g0iLPlot,S0iLPlot,P0iLPlot,0,-LnL_guess_fixed_g0,type(State_L_previous(end)));
        
        [PR_end_plot]=field_2Stiffness_nonIterative_P_EffectsOfRandomness(State_R_previous,State_R_previous(end),EE,DE,G,Sm,gm,Y,0,ET,DT,tm,...
