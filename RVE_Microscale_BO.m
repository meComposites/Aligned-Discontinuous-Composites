...
function [Ecomposite,MSScurves,alphag,alphac,MminOverlap,MminOverlapType,RVE_data,tm] = RVE_Microscale_BO(RVE_loop_counter,deltaEpsilon,Vf,lf,pc,pInterface,pcFailed,pg,pgPartial,pgFailed,HalfLengthC,HalfLengthG,dg,EgAvg,EgCoV,Sg,dc,EcAvg,EcCoV,Sc,Vc,G,RVEg,first_fibre_overlap,n_rows,n_columns,HiPerDiF_fuzzyness,Fuzzy_Factor,x_section_arrangement,max_strain,GiicmAvg,SmAvg,SmCoV,gmAvg,c_width,n_columns_c,mcWei,mgWei,lcWei,lgWei,Sc_frag_const,lf_SPSL,lf_hybrid,libGen,AA,AAE,AAde, AB,ABE,ABde, BB,BBE,BBde, ds, lLib,rng_switch,failure_modes_switch)
...
% assert switch = 1 stores location of interfaces where the overlap model
% couldn't converge and had to resort to short or long-fibre strategy
assert_switch = 0;

if pc==0 && pg ==0 && pInterface == 0 && pcFailed ==0 && pgFailed == 0
    defects_switch = 0;
else
    defects_switch = 1;
end
...
[xsp1, ia1, ~]=unique(round(100000*[lfWeiC/10+fieldx1(n,:)])/100000);
XBsp1=[fieldXB1(n,:)];
XBsp1=XBsp1(ia1);
xsp1(isnan(xsp1)) = [];
XBsp1(isnan(XBsp1)) = [];

[xsp2, ia2, ~]=unique(round(100000*[2*lfWeiC/10+fieldx2(n,:)])/100000);
XBsp2=[fieldXB2(n,:)];
XBsp2=XBsp2(ia2);
xsp2(isnan(xsp2)) = [];
XBsp2(isnan(XBsp2)) = [];

[xsp3, ia3, ~]=unique(round(100000*[3*lfWeiC/10+fieldx3(n,:)])/100000);
XBsp3=[fieldXB3(n,:)];
XBsp3=XBsp3(ia3);
xsp3(isnan(xsp3)) = [];
XBsp3(isnan(XBsp3)) = [];

[xsp4, ia4, ~]=unique(round(100000*[4*lfWeiC/10+fieldx4(n,:)])/100000);
XBsp4=[fieldXB4(n,:)];
XBsp4=XBsp4(ia4);
xsp4(isnan(xsp4)) = [];
XBsp4(isnan(XBsp4)) = [];
...
% XB4(isnan(XB4)) = [];
% FJ BO: stress field extrapolated to ensure that x and xsp give a
% consistent matrix size (peak does not occur at a point in x) - intervals
% for xsp also increased to 100000 to give better resolution on stress peak
XB1=interp1(xsp1,XBsp1,x,'pchip','extrap');
XB1=XB1(1:20001);
XB2=interp1(xsp2,XBsp2,x,'pchip','extrap');
XB2=XB2(1:40001);
XB3=interp1(xsp3,XBsp3,x,'pchip','extrap');
XB3=XB3(1:60001);
XB4=interp1(xsp4,XBsp4,x,'pchip','extrap');
XB4=XB4(1:80001);
...
n=7;
% real complex shape
[xsp, ia, ~]=unique(round(100000*[lfWeiG/10+fieldx(n,:) 3*lfWeiG/10+fieldx(n,:)])/100000);
XBsp=[fieldXB(n,:) fieldXB(n,end:-1:1)];
XBsp=XBsp(ia);
xsp(isnan(xsp)) = [];
XBsp(isnan(XBsp)) = [];

[xsp1, ia1, ~]=unique(round(100000*[lfWeiG/10+fieldx1(n,:)])/100000);
XBsp1=[fieldXB1(n,:)];
XBsp1=XBsp1(ia1);
xsp1(isnan(xsp1)) = [];
XBsp1(isnan(XBsp1)) = [];

[xsp2, ia2, ~]=unique(round(100000*[2*lfWeiG/10+fieldx2(n,:)])/100000);
XBsp2=[fieldXB2(n,:)];
XBsp2=XBsp2(ia2);
xsp2(isnan(xsp2)) = [];
XBsp2(isnan(XBsp2)) = [];

[xsp3, ia3, ~]=unique(round(100000*[3*lfWeiG/10+fieldx3(n,:)])/100000);
XBsp3=[fieldXB3(n,:)];
XBsp3=XBsp3(ia3);
xsp3(isnan(xsp3)) = [];
XBsp3(isnan(XBsp3)) = [];

[xsp4, ia4, ~]=unique(round(100000*[4*lfWeiG/10+fieldx4(n,:)])/100000);
XBsp4=[fieldXB4(n,:)];
XBsp4=XBsp4(ia4);
xsp4(isnan(xsp4)) = [];
XBsp4(isnan(XBsp4)) = [];
...
% ### FJ - na modified to support naxnb rectangle RVE ###
Mfail=cell(n_rows_outer_layer,n_columns_outer_layer);
MSScurves=Mfib;
...
switch defects_switch
    ...
    otherwise
        RVE_data.Pre_defect_stiffness=[];
end

% data storage for debugging of non-robust HBaM calls
switch assert_switch
    case 1
        RVE_data.Interface_assert_flag_record=zeros(2*n_rows_outer_layer-1,n_columns_outer_layer);
        RVE_data.Interface_assert_data_record=cell(2*n_rows_outer_layer-1,n_columns_outer_layer);
        ...
        for k=1:nCrack
            if Icracks(2,k)==Icracks(2,k+1)
                ...
                if libGen == 2
                    ...
                    if assert_switch==1 && assert_flag==1
...
%% Collection of interface properties for Effects of Randomness study
    function [xInt,yInt,EInt,deInt,assert_flag]=SSInterface_EffectsOfRandomness(EE_int,DE_int,ET_int,DT_int,signDeltaK,tm_int,Sm_int,lk)
        ...
        elseif tm_int <= 5E-04
            ...
            [EInt,deInt,xInt,yInt,assert_flag] = HBaMBiLinSP_robust_FJPostProcess_update_AvgIntervals(EE_int,signDeltaK*DE_int,ET_int,signDeltaK*DT_int,tm_int,lk,Sm_int,GiicmAvg,G,1000); % can have large deltaSigma because stress-strain curves are largely linear - use SPSL_VariableGiicSm.m for platelet cases
            %         xInt=xInt*100;
