function [xInt,yInt,EInt,deInt] = SS_HBaM(EE,DE,ET,DT,tm,L,SmIn,Gic,G)

% L     Length of the overlap
% e.g.
% SS_HBaM(933000,587000,0.017/4,0.003/4,0.01,1.5,80,1,1500)
% SS_HBaM(933000,787000,0.017/4,0.003/4,0.01,1.5,80,1,1500)
...
% Constitutive law
gm=[SmIn/G SmIn/G+(2*Gic/tm-(SmIn*SmIn/G))/SmIn 10^3]';
Sm=[SmIn 0 0]';
...
return;
    ...
    function [epsilon_inf, sigma_inf] = post_process(PsiR,gL,gR)
        if AL==0
            ...
            epsilon_inf=100*(tm/(2*L)*((gL*(EE+DE)*(ET-DT)+gR*(EE-DE)*(ET+DT))/(EE*ET+DE*DT))+2*sigma_inf*ET/(EE*ET+DE*DT));
        else
            sigma_inf=PsiR/(2*ET/(ET+DT)-K);
            epsilon_inf=100*(tm/(2*L)*((gL*(EE+DE)*(ET+DT)+gR*(EE-DE)*(ET-DT))/(EE*ET+DE*DT))+2*sigma_inf*ET/(EE*ET+DE*DT));
        end
...
%     end
    ...
    function P_err_123 = P1_F_123(g0_guess)
        t0_guess=g0_guess*G(1);
        %P[1_PsiL=-2/(Y(1)*Teq)*sqrt(SmIn^2-t0_guess^2); 12/08
        P1_PsiL=-2/(Y(1)*Teq)*sqrt(SmIn^2-t0_guess^2);
        P1_l1=2/Y(1)*atanh(sqrt((SmIn-t0_guess)/(SmIn+t0_guess)));
        P1_l1(P1_l1==Inf)=1/Y(1)*log(2*SmIn/t0_guess);
        ...
        %P1_lL2=2/Y(2)*atan(1/(SmIn)*sqrt((Y(2)/Y(1))^2*(SmIn^2-t0_guess^2)+(SmIn^2))+(Y(2)*Teq/2/(SmIn))*P1_PsiL);
%        P1_lL2=2/Y(2)*atan(1/(SmIn)*sqrt((Y(2)/Y(1))^2*(SmIn^2-t0_guess^2)+(SmIn^2))-(Y(2)*Teq/2/(SmIn))*P1_PsiL);
        P1_lL2=2/Y(2)*atan(Y(2)*Teq/(2*(SmIn))*(sqrt(P1_PsiL^2+(2/Y(2)/Teq)^2*(SmIn^2))-P1_PsiL));
        P1_PsiL2=sqrt(P1_PsiL^2+(2/(Y(2)*Teq))^2*(SmIn^2));
        
        P1_PsiR2=-P1_PsiL2/p;
        
        %P1_tR=sqrt(SmIn^2-((Y(1)*Teq*P1_PsiR2)/2)^2); 12/08
