function [gcritMax] = gMatrix_Bilinear_EffectsOfRandomness(N,type,Y,gm,Sm,ET,DT,tm,EE,DE)
...
%                 [length_act,jkl,~]= fminsearch(@find_gcrit,L/2,opts);
                [gcrit,~,~]= fminbnd(@find_gcrit,gini,gm(a),opts);
    ...
    function find_gcrit = find_gcrit(gcrit_guess)
        ...
        for j=a:b
            if j==a
                ...
                %S0=interp1q([0; gm],[0; Sm],gcrit_guess);
            	S0=(gcrit_guess<=gm(1))*gcrit_guess/gm(1)*Sm(1)+(gm(1)<gcrit_guess)*(gcrit_guess<gm(2))*Sm(1)*(1-(gcrit_guess-gm(1))/(gm(2)-gm(1)));
            else
                g0=gm(j-1);
                S0=Sm(j-1);
            end
